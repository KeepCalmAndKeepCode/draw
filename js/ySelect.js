(function ($) {
      ...
      buildOptions: function ($element) {
        ...
        $element.children().each(function (i, el) {
          ...
          if ('optgroup' == $el.prop('nodeName').toLowerCase()) {
            ...
            choices += `
            <div class="fs-option ${selected}"
              data-value="${$el.prop('value')}"
              data-level="${el.dataset.level}"
              style="padding-left:${15*el.dataset.level}px"
            >
              <span class="fs-checkbox"><i></i></span>
              <div class="fs-option-label">${$el.html()}</div>
            </div>`
            
            // '<div class="fs-option' + selected + '" data-value="' + $el.prop('value') + '"  data-level="'+ el.dataset.level +'"><span class="fs-checkbox"><i></i></span><div class="fs-option-label">' + $el.html() + '</div></div>';
  ...
  $.fn.ySelectedValues = function (splitString) {
    ...
    for (var i = 0; i < $selects.length; i++) {
      ...
      selectsArr.push($selects[i].value)
    }
    return selectsArr;
  }
