; (function ($) {
  var defaults = { id: "", data: [], fold: true, multiple: false, check: function () { }, done: function () { } }; $.fn.tagTree = function (options) {
    var that = $(this); options.id = "#" + that.attr("id");
    ...
    $(defaults.id + ' li.li-top > span').on('click', function (e) {
      var children = $(this).parent('li.li-top').find(' > ul > li'); if (children.is(":visible")) { children.hide('fast'); } else { children.show('fast'); }
      return false;
    });
    ...
    $(defaults.id + ' li span i').click(function (event) {
      ...
      if ($(this).hasClass('i-check')) {
        ...
        $(this).addClass('i-check'); defaults.check($(this).attr("data-val"));
        return false;
      }
    }); 
    defaults.done();
